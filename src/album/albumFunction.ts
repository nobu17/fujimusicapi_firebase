...
export default class AlbumFunction {
    ...
    switch (req.method) {
      ...
      case "DELETE":
        try {
          ...
          console.error("handle delete error", err);
          res.status(503).send({ error: "unexpected error" });
        }
        break;
      default:
  ...
  async handleDelete(req: functions.https.Request, res: functions.Response) {
    const reqGetter = new HttpReqGetter();
    const inputParam = reqGetter.getAlbumDeleteRequest(req);
    if (!inputParam) {
      res.status(400).send({ error: "paramError" });
      return;
    }
    if (inputParam.getMethodType() === "albumDelete") {
      const serv = new AlbumService();
      const result = await serv.deleteAlbum(inputParam as AlbumDeleteRequest);
  ...
  async handlePost(req: functions.https.Request, res: functions.Response) {
    ...
    if (inputParam.getMethodType() === "albumPost") {
      const serv = new AlbumService();
      const result = await serv.postAlbum(inputParam as AlbumPostRequest);
      console.log("service result:", result);
      switch (result.errorType) {
        case AlbumPostErrorType.none:
        case AlbumPostErrorType.partialError:
